...
module.exports = class NewUiAnnouncement extends PureComponent {
  ...
  switchToNewUi = async () => {
    const flag = 'betaUI'
    const enabled = true
    await this.props.dispatch(actions.setFeatureFlag(flag, enabled))
    await this.close()
    global.platform.openExtensionInBrowser()
  }

  render () {
    return h('div.new-ui-announcement', [
      ...
      h('section.new-ui-announcement__body', [
        h('h1', 'A New Version of Badger'),
        h('p', [
          "We're excited to announce a brand-new version of Badger with enhanced features and functionality.",
        ]),
        h('div.updates-list', [
        ...
        h('p', [
          'You can still use the current version of Badger. The new version is still in beta, ' +
      ...
      h('section.new-ui-announcement__footer', [
        h('h1', 'Ready to try the new Badger?'),
        h(
          'button.positive',
          {
